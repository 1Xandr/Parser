...
def get_data(url: str):
    ...
    desrciprion = desrciprion.text.replace("&nbsp;", " ") if desrciprion else None
    ...
    for item in instruction_info:
        text = item.find("span", class_="emotion-6kiu05").text.replace("\xa0", " ").replace("\xad", " ")
        # if dish has no image -> div of number change
        number = item.find("div", class_="emotion-19fjypw")
        number = item.find("span", class_="emotion-bzb65q").text if not number else number.find("span").text
        # if dish has no image -> None
        image = item.find("div", class_="emotion-1x955v4")

        try:
            image = image.find("img").get("src") if image else None
        except AttributeError:
            image = None  # This step has no image to get

        instruction_data.append([text, number, image])
