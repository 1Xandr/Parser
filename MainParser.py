...
def get_info():
    ...
    for page in range(1, 715):  # how many page we need to parse | site have 714 pages
        ...
        for dish in data:  # how many dish we have | one page = 14 dishes
            ...
            name_dish = dish.find("div", class_='emotion-1eugp2w').text
            ...
            yield name_dish, img_dish, url_dish, categories_dish, cuisine_dish
...
for data_dish in get_info():
    ...
    instruction = dish_info[1]  # list -> [[text, number, image], [text, number, image]]
    ...
    meat_grinder = equipment[8]

    # filling into MySql database
    fill_list_of_dishes(id_sql, name, image, description, cook_time, cuisine, categories)
    fill_energy_value(id_sql, calories, protein, fat, carbohydrate)

    for step in instruction:
        fill_instruction(id_sql, step[1], step[0], step[2])  # id, [text, number, image]
